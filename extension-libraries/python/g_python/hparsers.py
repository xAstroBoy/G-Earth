"""Typed parsers for common Habbo packets.

Drop-in compatible with the official ``g_python.hparsers`` plus:
  * ``HScoreBoard`` / ``HScoreEntry`` (used by many scripts, missing upstream),
  * a ``typeId`` alias on furni parsers,
  * fixed enum definitions (upstream ``HGroupMode`` / ``HBubble`` used ``:``
    instead of ``=`` and were silently empty),
  * ``__repr__`` on the data types.
"""
...
class HPoint:
    ...
    def __eq__(self, other: object) -> bool:
        return isinstance(other, HPoint) and (self.x, self.y, self.z) == (other.x, other.y, other.z)
...
class HEntity:
    ...
    def __repr__(self) -> str:
        return "HEntity(id={}, index={}, name={!r}, type={})".format(
            self.id, self.index, self.name, self.entity_type.name)
    ...
    @classmethod
    def parse(cls, packet: HPacket) -> List["HEntity"]:
...
class HUserUpdate:
    ...
    def __str__(self) -> str:
        return ("<HUserUpdate> [{}] - X: {} - Y: {} - Z: {} - head {} - body {} - next tile {}"
                .format(self.index, self.tile.x, self.tile.y, self.tile.z,
                        self.headFacing.name, self.bodyFacing.name, self.nextTile))
    ...
    @classmethod
    def parse(cls, packet: HPacket) -> List["HUserUpdate"]:
...
# ---------------------------------------------------------------------- #
# Furniture "stuff-data" helper
# ---------------------------------------------------------------------- #
def read_stuff(packet: HPacket, category: int) -> list:
    stuff: list = []
    cat2 = category & 0xFF
    ...
    if cat2 == 6:  # highscores
        ...
        stuff.append([(packet.read_int(), [packet.read_string() for _ in range(packet.read_int())])
...
class HFloorItem:
    def __init__(self, packet: HPacket):
        ...
        self.owner: Optional[str] = None  # filled in by parse()

        if self.type_id < 0:
            packet.read_string()

    #: Alias kept for scripts/parsers that use the camelCase name.
    @property
    def typeId(self) -> int:  # noqa: N802 (compat alias)
        return self.type_id

    def __repr__(self) -> str:
        return "HFloorItem(id={}, type_id={}, tile={!r})".format(self.id, self.type_id, self.tile)

    @classmethod
    def parse(cls, packet: HPacket) -> List["HFloorItem"]:
        owners: Dict[int, str] = {}
        for _ in range(packet.read_int()):
        ...
        for furni in furnis:
            furni.owner = owners.get(furni.owner_id)
        return furnis


class HWallItem:
    def __init__(self, packet: HPacket):
        self.id, self.type_id, self.location, self.state, self.seconds_to_expiration, \
            self.usage_policy, self.owner_id = packet.read("sissiii")
    ...
    def __repr__(self) -> str:
        return "HWallItem(id={}, type_id={}, location={!r})".format(
            self.id, self.type_id, self.location)

    @classmethod
    def parse(cls, packet: HPacket) -> List["HWallItem"]:
        owners: Dict[int, str] = {}
        for _ in range(packet.read_int()):
            owner_id = packet.read_int()
            owners[owner_id] = packet.read_string()
...
class HWallUpdate:
    """Parses a MoveWallItem packet (id / coord string)."""
    def __init__(self, packet: HPacket):
...
class HUserProfile:
    def __init__(self, packet: HPacket):
        self.id, self.username, self.figure, self.motto, self.creation_date, \
            self.achievement_score, self.friend_count, self.is_friend, \
            self.is_requested_friend, self.is_online = packet.read("issssiiBBB")
    ...
    def __str__(self) -> str:
        return ("id: {}, username: {}, score: {}, friends: {}, online: {}, groups: {}, "
                "level: {}, gems: {}".format(self.id, self.username, self.achievement_score,
                                             self.friend_count, self.is_online, len(self.groups),
                                             self.level, self.gems))
...
class HInventoryItem:
    def __init__(self, packet: HPacket):
        ...
        self.is_groupable, self.is_tradeable, _, self.market_place_allowed, \
            self.seconds_to_expiration, self.has_rent_period_started, self.room_Id = \
            packet.read("BBBBiBi")
    ...
    @classmethod
    def parse(cls, packet: HPacket) -> List["HInventoryItem"]:
        total, current = packet.read("ii")
        return [HInventoryItem(packet) for _ in range(packet.read_int())]


# ---------------------------------------------------------------------- #
# Scoreboard (compat with _Library.Entity.HScoreBoard)
# ---------------------------------------------------------------------- #
class HScoreEntry:
    def __init__(self, score: int, username: str):
        self.score = score
    ...
    def __repr__(self) -> str:
        return "HScoreEntry({}, {!r})".format(self.score, self.username)


class HScoreBoard:
    """Parses a scoreboard furni's FURNITURE_DATA payload.

    Header structure ``sisii`` (id-as-string, ...), then an int count of
    entries, each ``iis`` (score, type_id, username). Only ``type_id == 1``
    entries with a non-empty username are kept.
    """
    def __init__(self, packet: HPacket):
        (self.id, _d1, _d2, _d3, _d4) = packet.read("sisii")
        self.entries: Dict[str, HScoreEntry] = {}
        self.ListEntries: List[HScoreEntry] = []

        num_entries = packet.read_int()
        for _ in range(num_entries):
            score, type_id, username = packet.read("iis")
            if type_id != 1 or username == "":
                continue
            entry = HScoreEntry(score, username)
            self.entries[username.lower()] = entry
            self.ListEntries.append(entry)

    def GetPlayerScore(self, username: str) -> Optional[HScoreEntry]:
        return self.entries.get(username.lower())

    def GetScores(self) -> List[HScoreEntry]:
        return self.ListEntries

    def __str__(self) -> str:
        return "id: {}, ScoreEntries: {}".format(self.id, self.entries)

    @classmethod
    def parse(cls, packet: HPacket) -> "HScoreBoard":
